...
function render(trace; ymax=0.02)
    ...
    k = choices["k"]
    bounds = vcat([0.], sort([choices["cp$i"] for i=1:k]), [T])
    rates = [choices["h$i"] for i=1:k+1]
...
const observations = DynamicChoiceTrie()
...
function get_rate_vector(trace, test_points)
    choices = get_choices(trace)
    k = choices["k"]
    cps = [choices["cp$i"] for i=1:k]
    hs = [choices["h$i"] for i=1:k+1]
    rate = Vector{Float64}()
    cur_h_idx = 1
    cur_h = hs[cur_h_idx]
    next_cp_idx = 1
