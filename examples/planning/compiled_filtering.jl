...
# the state is the 'dist'
@compiled @gen function kernel(t::Int, prev_state::KernelState, params::KernelParams)
    # NOTE: if t = 1 then prev_dist can be NaN
    dist::Float64 = @addr(normal(
        t > 1 ? (prev_state.dist + (times[t] - times[t-1]) * params.speed) : params.speed * times[1],
        params.dist_slack), :dist)
...
#@compiled @gen function model(step::Int, path::Path, times::Vector{Float64}, speed::Float64,
                    #noise::Float64, dist_slack::Float64)

    # walk path and generate noisy observations as you go..
 	#distances_from_start::Vector{Float64} = compute_distances_from_start(path)
...
function show_prior_samples()
    ...
    for i=1:15
        subplot(4, 4, i)
        ax = gca()
        trace = simulate(hmm, (length(times), model_args_rest...))
        render(scene, start, stop, maybe_path, times, speed, noise, dist_slack, trace, ax)
    end
    savefig("compiled_filtering.png")
end
show_prior_samples()
...
function experiment()
    ...
    for num_particles in [100]#[10, 100, 1000, 1000]
        ...
        for rep=1:5
            @time (traces, _, lml) = particle_filter(hmm, model_args_rest, length(times),
