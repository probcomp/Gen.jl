...
function process!(rng::AbstractRNG,
                  ...
                  selection::Selection, 
                  state::SwitchRegenerateState{T}) where {C, N, K, T}
    new_trace, weight, retdiff = regenerate(rng, getfield(state.prev_trace, :branch), args, kernel_argdiffs, selection)
    state.index = index
    state.weight = weight
    state.noise = project(new_trace, EmptySelection()) - project(state.prev_trace, EmptySelection())
    state.score = get_score(new_trace)
...
function regenerate(rng::AbstractRNG,
    ...
    process!(rng, gen_fn, index, index_argdiff, args[2 : end], argdiffs[2 : end], selection, state)
