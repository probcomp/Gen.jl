...
function process!(rng::AbstractRNG, gen_fn::Map{T,U}, args::Tuple, choices::ChoiceMap,
...
function generate(rng::AbstractRNG, gen_fn::Map{T,U}, args::Tuple, choices::ChoiceMap) where {T,U}
    len = length(args[1])
    ...
    for key=1:len
        process!(rng, gen_fn, args, choices, key, state)
    end
