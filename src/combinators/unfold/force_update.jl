mutable struct UnfoldForceUpdateState{T,U}
    init_state::T
    weight::Float64
    score::Float64
    noise::Float64
...
function process_retained!(gen_fn::Unfold{T,U}, params::Tuple,
                           ...
                           state::UnfoldForceUpdateState{T,U}) where {T,U}
    local subtrace::U
...
function force_update(args::Tuple, ::NoArgDiff,
                      trace::VectorTrace{UnfoldType,T,U},
                      assmt::Assignment) where {T,U}
    argdiff = UnfoldCustomArgDiff(false, false)
    force_update(args, argdiff, trace, assmt)
end

function force_update(args::Tuple, ::UnknownArgDiff,
                      trace::VectorTrace{UnfoldType,T,U},
                      assmt::Assignment) where {T,U}
...
function force_update(args::Tuple, argdiff::UnfoldCustomArgDiff,
                      trace::VectorTrace{UnfoldType,T,U},
                      assmt::Assignment) where {T,U}
    gen_fn = trace.gen_fn
    ...
    state = UnfoldForceUpdateState{T,U}(init_state, -score_decrement, score, noise,
