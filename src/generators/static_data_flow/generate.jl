struct StaticDataFlowGenerateState
    trace::Symbol
    score::Symbol
    weight::Symbol
...
function process!(ir::DataFlowIR, state::StaticDataFlowGenerateState, node::JuliaNode)
    trace = state.trace
...
function process!(ir::DataFlowIR, state::StaticDataFlowGenerateState,
...
function process!(ir::DataFlowIR, state::StaticDataFlowGenerateState, node::AddrDistNode)
    ...
    if isa(schema, StaticAddressSchema) && (addr in leaf_node_keys(schema))
        ...
        push!(state.stmts, Expr(:(=), value, Expr(:call, :static_get_leaf_node, :constraints, Expr(:call, :Val, QuoteNode(addr)))))
        push!(state.stmts, Expr(:(=), increment, Expr(:call, :logpdf, dist, value, args...)))
    ...
    elseif isa(schema, StaticAddressSchema) || isa(schema, EmptyAddressSchema)
        push!(state.stmts, Expr(:(=), value, Expr(:call, :random, dist, args...)))
        push!(state.stmts, Expr(:(+=), score, Expr(:call, :logpdf, dist, value, args...)))
    else
...
function process!(ir::DataFlowIR, state::StaticDataFlowGenerateState, node::AddrGeneratorNode)
    trace, score, weight, schema = (state.trace, state.score, state.weight, state.schema)
    addr = node.address
...
function codegen_generate(gen::Type{T}, args, constraints) where {T <: StaticDataFlowGenerator}
    ...
    state = StaticDataFlowGenerateState(trace, score, weight, schema, stmts)
...
@generated function Gen.generate(gen::Gen.StaticDataFlowGenerator, args, constraints)
