...
constructing custom distributions, each of which are explained below:

1. The [`HeterogeneousMixture`](@ref) and [`HomogeneousMixture`](@ref) constructors
   for distributions that are mixtures of other distributions.

2. The [`ProductDistribution`](@ref) constructor for  distributions that are products of
   other distributions.

3. The [`@dist` constructor](@ref dist_dsl), for a distribution that can be expressed as a
...
4. An API for defining arbitrary [custom distributions](@ref
   custom_distributions_howto) in plain Julia code.
...
representing the distribution over _return values_ of `body`. Arguments are
optionally typed.
...
The [`has_argument_grads`](@ref) function is also part of the distribution API.
