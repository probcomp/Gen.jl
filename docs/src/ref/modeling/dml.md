# [Built-in Modeling Language](@id dml)

Gen provides a built-in modeling language for defining generative functions, using a syntax that extends Julia's syntax for defining regular Julia functions. This language is also referred to as the **Dynamic Modeling Language (DML)**.

Generative functions in this modeling language are identified using the `@gen` keyword in front of a Julia function definition.
...
See [Generative Functions](@ref gfi) for the full set of operations supported by a generative function.
