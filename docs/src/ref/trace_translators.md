...
This probability would be needed if, for example, `p2` defines a probabilistic model and we want to construct a proposal distribution by sampling a trace `t1` from `p1` and then applying a deterministic transform to obtain a proposed trace `t2` for `p2`.
